...
public:
	QueryResult(std::string s, std::shared_ptr<StrVec> svec, std::shared_ptr<std::set<size_type>> sset) :word(s), file(svec), line_number(sset) {}      //找到指定string时的构造函数
	...
	std::shared_ptr<StrVec> get_file()const { return file; }                     //获取保存的输入文件
private:
	std::string word;                                   //要查询的单词
	std::shared_ptr<StrVec> file;     //输入的文件
...
private:
	std::shared_ptr<StrVec> input_file;                           //使用StrVec类保存输入的每一行文本
